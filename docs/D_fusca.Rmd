...
```{r include=FALSE}
Dampiera_fusca <- raster(here("Input/Dampiera_fusca.tif"))
```
...
```{r message=FALSE, warning=FALSE, include=FALSE}
D_fusca <- leaflet(options = leafletOptions(minZoom = 5)) %>%              # Set extent of zoom.
  
  addTiles() %>%                                                                # Add basemap.
  
  addRasterImage(Dampiera_fusca,                                         # Add SDM.
                 colors   = pal,
                 group    = "Dampiera fusca",
                 layerId  = "Dampiera fusca", 
                 opacity  = 0.8,
  ...
  addLayersControl(overlayGroups = c("Dampiera fusca",                   # To turn layers on and off.
                                     "TSF",
                                     "Fire Freq"),
