...
```{r include=FALSE}
Hoplocephalus_stephensii <- raster(here("Input/Hoplocephalus_stephensii.tif"))

crs(Hoplocephalus_stephensii) <- 4326
```
...
pal <- colorNumeric(palette  = "viridis",
                    values(Hoplocephalus_stephensii),
...
```{r message=FALSE, warning=FALSE, include=FALSE}
H_stephensii <- leaflet(options = leafletOptions(minZoom = 5)) %>%              # Set extent of zoom.
  
  addTiles() %>%                                                                # Add basemap.
  
  addRasterImage(Hoplocephalus_stephensii,                                         # Add SDM.
                 colors   = pal,
                 group    = "Hoplocephalus stephensii",
                 layerId  = "Hoplocephalus stephensii", 
                 opacity  = 0.8,
  ...
  addLegend(pal    = pal,                                                       # Add legend for each layer.
            group  = "Hoplocephalus stephensii", 
            values = values(Hoplocephalus_stephensii),
  ...
  addLayersControl(overlayGroups = c("Hoplocephalus stephensii",                   # To turn layers on and off.
                                     "TSF",
                                     "Fire Freq"),
