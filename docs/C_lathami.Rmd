...
```{r include=FALSE}
Calyptorhynchus_lathami <- raster(here("Input/Calyptorhynchus_lathami.tif"))
```
...
pal <- colorNumeric(palette  = "viridis",
                    values(Calyptorhynchus_lathami),
...
```{r message=FALSE, warning=FALSE, include=FALSE}
C_lathami <- leaflet(options = leafletOptions(minZoom = 5)) %>%              # Set extent of zoom.
  
  addTiles() %>%                                                                # Add basemap.
  
  addRasterImage(Calyptorhynchus_lathami,                                         # Add SDM.
                 colors   = pal,
                 group    = "Calyptorhynchus lathami",
                 layerId  = "Calyptorhynchus lathami", 
                 opacity  = 0.8,
  ...
  addLegend(pal    = pal,                                                       # Add legend for each layer.
            group  = "Calyptorhynchus lathami", 
            values = values(Calyptorhynchus_lathami),
  ...
  addLayersControl(overlayGroups = c("Calyptorhynchus lathami",                   # To turn layers on and off.
                                     "TSF",
                                     "Fire Freq"),
