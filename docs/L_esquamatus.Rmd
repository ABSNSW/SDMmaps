...
```{r include=FALSE}
Leucopogon_esquamatus <- raster(here("Input/Leucopogon_esquamatus.tif"))
```
...
```{r message=FALSE, warning=FALSE, include=FALSE}
L_esquamatus <- leaflet(options = leafletOptions(minZoom = 5)) %>%              # Set extent of zoom.
  
  addTiles() %>%                                                                # Add basemap.
  
  addRasterImage(Leucopogon_esquamatus,                                         # Add SDM.
                 colors   = pal,
                 group    = "Leucopogon esquamatus",
                 layerId  = "Leucopogon esquamatus", 
                 opacity  = 0.8,
  ...
  addLegend(pal    = pal,                                                       # Add legend for each layer.
            group  = "Leucopogon esquamatus", 
            values = values(Leucopogon_esquamatus),
  ...
  addLayersControl(overlayGroups = c("Leucopogon esquamatus",                   # To turn layers on and off.
                                     "TSF",
                                     "Fire Freq"),
