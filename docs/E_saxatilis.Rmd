...
```{r include=FALSE}
Eucalyptus_saxatilis <- raster(here("Input/Eucalyptus_saxatilis.tif"))
```
...
```{r message=FALSE, warning=FALSE, include=FALSE}
E_saxatilis <- leaflet(options = leafletOptions(minZoom = 5)) %>%              # Set extent of zoom.
  
  addTiles() %>%                                                                # Add basemap.
  
  addRasterImage(Eucalyptus_saxatilis,                                         # Add SDM.
                 colors   = pal,
                 group    = "Eucalyptus saxatilis",
                 layerId  = "Eucalyptus saxatilis", 
                 opacity  = 0.8,
  ...
  addLegend(pal    = pal,                                                       # Add legend for each layer.
            group  = "Eucalyptus saxatilis", 
            values = values(Eucalyptus_saxatilis),
  ...
  addLayersControl(overlayGroups = c("Eucalyptus saxatilis",                   # To turn layers on and off.
                                     "TSF",
                                     "Fire Freq"),
